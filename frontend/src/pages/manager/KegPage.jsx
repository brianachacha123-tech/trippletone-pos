    ...
    <div>
      ...
      {showTxModal && (
        <div className="modal-overlay" onClick={() => setShowTxModal(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h2>Add Daily Keg Record - {showTxModal.product_name}</h2>
            <form onSubmit={handleAddTransaction}>
              ...
              <div className="form-group"><label>Till Amount (KSh)</label><input type="number" step="0.01" value={txForm.till} onChange={e => setTxForm({...txForm, till: e.target.value})} /></div>
      ...
      {activeKeg && (
        <div className="modal-overlay" onClick={() => setActiveKeg(null)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ width: '600px' }}>
            <h2>Keg History: {activeKeg.product_name}</h2>
            <p style={{ marginBottom: '16px', color: '#666' }}>Total Revenue: {fmt(activeKeg.total_revenue)} | Profit: {fmt(activeKeg.profit)}</p>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  ...
                  <th style={{ padding: '8px', textAlign: 'right', borderBottom: '2px solid #eee', fontSize: '12px' }}>Daily Total</th>
                </tr>
              </thead>
              <tbody>
                {txList.map(tx => (
